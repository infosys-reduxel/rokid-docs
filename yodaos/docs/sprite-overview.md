...
> This FAQ was updated from the upstream `developerdoc.rokid.com/sprite` page (fetched 2026-06-08). The upstream page focuses on CXR-L and bare-metal development; the earlier CXR-M/CXR-S Q&As have been replaced by the content below. Maven `client-l` is at 1.1.2 as of 2026-08-28 (verified via `maven-metadata.xml`); `developerdoc.rokid.com/sprite` is a client-rendered SPA that could not be re-checked from this environment as of 2026-09-04, so it is unknown whether the portal FAQ has been updated to reflect this version.
