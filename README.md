...
| **Maven Artifact (latest)** | `com.rokid.cxr:client-l:1.1.1` (2026-08-14) |
| **Size** | 171,369 bytes (1.1.1 AAR); 70,543 bytes (1.0.4 AAR) |
...
`CXRLink` extends `ExternalAppClient`, which binds to `IMediaStreamService` via Android AIDL for media streaming and AI app integration. This API is unchanged as of v1.1.1. v1.1.1 additionally introduces a separate coroutine-based `com.rokid.cxr.session` API (`CxrSessionManager` / `CxrSession`) alongside it — see the [Session API section](cxr-l/api-reference.md#session-api-v111-package-comrokidcxrsession) of the API reference.
